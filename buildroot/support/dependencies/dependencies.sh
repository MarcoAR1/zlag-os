...
# Support macOS clang (reports as "Apple clang version" not "gcc version")
COMPILER_VERSION=$($COMPILER -v 2>&1 | sed -n '/^gcc version/p' |
	sed -e 's/^gcc version \([0-9\.]\)/\1/g' -e 's/[-\ ].*//g' -e '1q')
if [ -z "$COMPILER_VERSION" ] ; then
	# Try to extract clang version (macOS compatibility)
	COMPILER_VERSION=$($COMPILER --version 2>&1 | head -n1 | sed -n 's/.*clang version \([0-9\.]*\).*/\1/p')
fi
if [ -z "$COMPILER_VERSION" ] ; then
	echo
	echo "You must install 'gcc' on your build machine";
